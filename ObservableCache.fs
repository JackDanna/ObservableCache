...
let temp (observableOfInput: IObservable<Input<'Item, 'ItemId, 'ItemMsg>>) (readItemOnDatabase: 'ItemId -> Async<Result<'Item, string>>) =
    observableOfInput
    |> AsyncSeq.ofObservableBuffered
    |> AsyncSeq.scanAsync 
        (
            fun (state: 'Item option) input ->
                match state with
                | None -> 
                    input.CacheInput
                    |> cacheInputToItemId
                    |> readItemOnDatabase
                | Some item -> async.Return item
                |>
